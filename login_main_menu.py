...
def mostrar_menu_principal():
    ...
    imagen_menu = Image.open("patita.png")  # Cambia por la ruta de tu imagen
    imagen_menu = imagen_menu.resize((150, 150))  # Ajusta el tamaño de la imagen
    imagen_menu_tk = ImageTk.PhotoImage(imagen_menu)
    
    label_imagen_menu = tk.Label(menu_principal, image=imagen_menu_tk)
    label_imagen_menu.image = imagen_menu_tk  # Evita que la imagen sea eliminada por el recolector de basura
    label_imagen_menu.pack(pady=10)
    ...
    def abrir_ventana():
        ...
        imagen_ventana = Image.open("perfil.png")
        imagen_ventana = imagen_ventana.resize((150, 250))
        imagen_ventana_tk = ImageTk.PhotoImage(imagen_ventana)
        label_imagen_ventana = tk.Label(nueva_ventana, image=imagen_ventana_tk,bg="white",fg="blue")
        label_imagen_ventana.image = imagen_ventana_tk 
        label_imagen_ventana.pack(pady=10)
        ...
        def create_pet_button(parent, text, image_path, x, y):
            pet_img = load_image(image_path, (10, 50))  # Ajustar tamaño de la imagen
            pet_button = Button(parent, text=text, image=pet_img, compound="left", width=265, height=70)
...
imagen_login = Image.open("loginfap.png")  # Cambia por la ruta de tu imagen
imagen_login = imagen_login.resize((250, 100))
imagen_login_tk = ImageTk.PhotoImage(imagen_login)

label_imagen_login = tk.Label(ventana_login, image=imagen_login_tk)
label_imagen_login.image = imagen_login_tk  # Evita que la imagen sea eliminada por el recolector de basura
